...
- (未着手) **箇条書きの各項目が1文に連結される**．句点で終わらない行は
  `join_lines()` が次の行につなげるので，design.md の「段階」の3項目が
  1文になった．箇条書きの項目は1項目1文として扱うか，実データで検討する．
- (未着手) 見出しをどう扱うか．見出しは前後の文と語を共有しにくいので，
  孤立文(デッドコード)として大量に挙がる(design.md では35文中の多くが見出し)．
  解析から外すか，段落の題として別に扱うか決める．
